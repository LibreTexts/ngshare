...
def test_add_instructor(http_client, base_url):
    ...
    hc, bu = http_client, base_url
    user = 'eric'
    yield from assert_fail(url + 'course2/lawrence', method='POST',
                msg='Permission denied (not course instructor)')
    user = 'root'
    yield from assert_fail(url + 'course9/lawrence', method='POST', msg='Course not found')
    data = {}
    yield from assert_fail(url + 'course2/lawrence', data=data, method='POST',
    ...
    assert len((yield from assert_success('/api/instructors/course2'))['instructors']) == 2
    # Test changing instructor name
    user = 'abigail'
    yield from assert_fail(url + 'course2/lawrence', data=data, method='POST',
                msg='Permission denied (cannot modify other instructors)')
    user = 'abigail'
    yield from assert_fail(url + 'course2/eric', data=data, method='POST',
                msg='Permission denied (cannot modify instructors)')
    user = 'abigail'
    yield from assert_fail(url + 'course2/kevin', data=data, method='POST',
                msg='Permission denied (cannot modify instructors)')
    user = 'lawrence'
    yield from assert_success(url + 'course2/lawrence', data=data, method='POST')
    ...
    yield from assert_fail(url + 'course1/lawrence', data=data, method='POST',
                msg='Permission denied (not course instructor)')
    user = 'root'
    yield from assert_success(url + 'course1/lawrence', data=data, method='POST')
    assert len((yield from assert_success('/api/instructors/course1'))['instructors']) == 2
...
def test_get_instructor(http_client, base_url):
    ...
    yield from assert_fail(url + 'course2/lawrence',
...
def test_delete_instructor(http_client, base_url):
    ...
    hc, bu = http_client, base_url
    user = 'abigail'
    yield from assert_fail(url + 'course2/lawrence', method='DELETE',
                msg='Permission denied (not admin)')
    user = 'root'
    yield from assert_fail(url + 'course9/lawrence', method='DELETE', msg='Course not found')
    yield from assert_fail(url + 'course2/eric', method='DELETE', msg='Instructor not found')
    yield from assert_success(url + 'course2/lawrence', method='DELETE')
    yield from assert_success(url + 'course1/kevin', method='DELETE')
    yield from assert_success(url + 'course1/kevin', method='POST',
                   data={'first_name': '', 'last_name': '', 'email': ''})
...
def test_list_instructors(http_client, base_url):
    ...
    yield from assert_fail(url + 'course2',
                msg='Permission denied (not related to course)')
    user = 'eric'
...
def test_add_student(http_client, base_url):
    ...
    yield from assert_fail(url + 'course2/abigail', data=data, method='POST',
    ...
    user = 'kevin'
    yield from assert_success(url + 'course1/lawrence', data=data, method='POST')
    assert len((yield from assert_success('/api/instructors/course1'))['instructors']) == 1
...
def test_delete_student(http_client, base_url):
    ...
    yield from assert_fail(url + 'course2/lawrence', method='DELETE',
                msg='Permission denied (not course instructor)')
    user = 'abigail'
    yield from assert_fail(url + 'course2/kevin', method='DELETE', msg='Student not found')
    yield from assert_success(url + 'course2/lawrence', method='DELETE')
...
def test_add_students(http_client, base_url):
    ...
    user = 'kevin'
    yield from assert_fail(url + 'course9', msg='Course not found')
    yield from assert_fail(url + 'course2', method='POST',
                msg='Permission denied (not course instructor)')
    yield from assert_fail(url + 'course1', method='POST', data={},
                msg='Please supply students')
    yield from assert_fail(url + 'course1', method='POST', data={'students': '"'},
                msg='Students cannot be JSON decoded')
    yield from assert_fail(url + 'course1', method='POST', data={'students': '12'},
                msg='Incorrect request format')
    yield from assert_fail(url + 'course1', method='POST', data={'students': '[]'},
                msg='Please supply students')
    yield from assert_fail(url + 'course1', method='POST', data={'students': '[1,2]'},
                msg='Incorrect request format')
    students = [{'username': 'a', 'email': 'b', 'first_name': 'c'}]
    yield from assert_fail(url + 'course1', method='POST',
    ...
    resp = yield from assert_success(url + 'course1', method='POST',
                          data={'students': json.dumps(students)})
    expected = [
...
def test_download_assignment(http_client, base_url):
    ...
    # Check list_only
    files = (yield from assert_success(url + 'course1/challenge?list_only=true'))['files']
    assert set(files[0]) == {'path', 'checksum'}
    assert files[0]['path'] == 'file2'
    assert files[0]['checksum'] == hashlib.md5(b'22222').hexdigest()
    user = 'eric'
    yield from assert_fail(url + 'course1/challenge',
                msg='Permission denied (not related to course)')
...
def test_release_assignment(http_client, base_url):
    ...
    user = 'kevin'
    yield from assert_fail(url + 'jkl/challenger', method='POST',
                data=data, msg='Course not found')
    yield from assert_fail(url + 'course1/challenger', method='POST',
                msg='Please supply files')
    yield from assert_success(url + 'course1/challenger', method='POST',
                   data=data)
    yield from assert_fail(url + 'course1/challenger', method='POST',
                data=data, msg='Assignment already exists')
    data['files'] = json.dumps([{'path': 'a', 'content': 'amtsCg'}])
    yield from assert_fail(url + 'course1/challenges', method='POST',
...
def test_delete_assignment(http_client, base_url):
    ...
    hc, bu = http_client, base_url
    user = 'lawrence'
    yield from assert_fail(url + 'course1/challenger', method='DELETE',
                msg='Permission denied (not course instructor)')
    user = 'kevin'
    yield from assert_fail(url + 'jkl/challenger', method='DELETE', msg='Course not found')
    yield from assert_fail(url + 'course1/challengers', method='DELETE',
...
def test_list_submissions(http_client, base_url):
    ...
    yield from assert_fail(url + 'course1/challenges',
                msg='Assignment not found')
    ...
    yield from assert_fail(url + 'course1/challenges',
                msg='Permission denied (not course instructor)')
    yield from assert_fail(url + 'course2/assignment2a',
                msg='Permission denied (not course instructor)')
...
def test_list_student_submission(http_client, base_url):
    ...
    assert len(result['submissions']) == 0
    user = 'kevin'
    yield from assert_fail(url + 'course2/assignment2a/eric',
                msg='Permission denied (not course instructor)')
    user = 'abigail'
    yield from assert_fail(url + 'course1/challenge/lawrence',
                msg='Permission denied (not course instructor)')
    user = 'lawrence'
...
def test_submit_assignment(http_client, base_url):
    ...
    data = {'files': json.dumps([{'path': 'a', 'content': 'amtsCg=='},
                                 {'path': 'b', 'content': 'amtsCg=='}])}
    yield from assert_fail(url + 'jkl/challenge', method='POST',
                msg='Course not found')
    yield from assert_fail(url + 'course1/challenges', method='POST',
                msg='Assignment not found')
    user = 'lawrence'
    yield from assert_fail(url + 'course1/challenge', method='POST',
                msg='Please supply files')
    resp1 = yield from assert_success(url + 'course1/challenge', method='POST', data=data)
    ts1 = MyHelpers().strptime(resp1['timestamp'])
    data['files'] = json.dumps([{'path': 'a', 'content': 'amtsCg=='}])
    resp2 = yield from assert_success(url + 'course1/challenge', method='POST', data=data)
...
def test_download_submission(http_client, base_url):
    ...
    user = 'abigail'
    yield from assert_fail(url + 'course2/assignment2a/eric', msg='Submission not found')
...
def test_upload_feedback(http_client, base_url):
    ...
    yield from assert_fail(url + 'jkl/challenge/st', method='POST', data=data,
                msg='Course not found')
    yield from assert_fail(url + 'course1/challenges/st', method='POST', data=data,
                msg='Assignment not found')
    yield from assert_fail(url + 'course1/challenge/st', method='POST', data=data,
                msg='Student not found')
    yield from assert_success(url + 'course1/challenge/lawrence', method='POST', data=data)
    data['files'] = json.dumps([{'path': 'c', 'content': 'amtsCf=='}])
    yield from assert_success(url + 'course1/challenge/lawrence', method='POST', data=data)
    yield from assert_fail(url + 'course1/challenge/lawrence', method='POST', data={},
                msg='Please supply timestamp')
    yield from assert_fail(url + 'course1/challenge/lawrence', method='POST',
                data={'timestamp': 'a'}, msg='Time format incorrect')
    user = 'abigail'
    yield from assert_fail(url + 'course2/assignment2a/eric', method='POST', data=data,
                msg='Submission not found')
    yield from assert_fail(url + 'course2/assignment2a/eric', method='POST',
...
def test_download_feedback(http_client, base_url):
    ...
    yield from assert_fail(url + 'course1/challenge/lawrence', params={},
                msg='Please supply timestamp')
    yield from assert_fail(url + 'course1/challenge/lawrence', params={'timestamp': 'a'},
                msg='Time format incorrect')
    user = 'eric'
    yield from assert_fail(url + 'course2/assignment2a/eric',
                params={'timestamp': timestamp}, msg='Submission not found')
    user = 'kevin'
    feedback = yield from assert_success(url + 'course1/challenge/lawrence',
                              params={'timestamp': timestamp})
...
def test_remove_course(http_client, base_url):
    ...
    hc, bu = http_client, base_url
    user = 'kevin'
    yield from assert_fail(url + 'course1', method='DELETE',
                msg='Permission denied (not admin)')
    user = 'root'
